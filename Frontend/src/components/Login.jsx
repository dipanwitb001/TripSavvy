...
const Login = () => {
  ...
  //  const userData = response.data.data; // this contains { username: "ssj", ... }
    ...
    if (!errors.username && !errors.email && !errors.password) {
      axios
        ...
        .then((res) => {
          ...
          const userData = res.data.user || { username: values.username };
          setUser(userData);

          setAccountCreated(true);
          setSuccessMessage('Account logged in successfully');
          setValues({
            username: '',
            email: '',
            password: '',
    ...
    <div>
      <div className="relative w-screen h-screen overflow-hidden">
        ...
        <div className="bg-slate-800 border border-slate-400 rounded-md p-8 shadow-lg backdrop-filter backdrop-blur-sm bg-opacity-30 relative
          w-11/12 max-w-md mx-auto my-auto top-1/2 transform -translate-y-1/2
          sm:w-9/12 sm:max-w-lg
          md:w-6/12 md:max-w-md
          lg:w-3/12
        ">
          {isAccountCreated && (
            <div>
              <div className="mb-4 p-2 text-green-400 rounded-md">{successMessage}</div>
              <Link to="/">
                <button
                  type="button"
                  className="w-full p-2 mt-4 text-white rounded-md bg-blue-500"
                >
                  Start Exploring
                </button>
                <button
          ...
          {!isAccountCreated && (
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4 flex flex-col sm:flex-row items-center justify-between gap-2">
                ...
                <Link to="/forget-password" className="text-blue-500 cursor-pointer">
                  Forget Password
                </Link>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                <button
                  type="submit"
                  className="flex-1 p-2 text-white rounded-md bg-blue-500"
                >
                  Login
                </button>
                <Link to="/SignIn" className="flex-1">
                  <button
                    type="button"
                    className="w-full p-2 text-white rounded-md bg-blue-500"
                  >
                    Sign Up
                  </button>
                </Link>
